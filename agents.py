...
class Bob:
  ...
  def distribute_all_shares(self):
    ...
    #   if self.is_cheater:
    #     self.bob_map[share[1]].receive_share((QubitPlaceholder(), None), share_idx)
    #   else:
    #     self.bob_map[share[1]].receive_share(share, share_idx)

    # print(self.shares)
    # if self.is_cheater:
      # self.shares = [[QubitPlaceholder(), share_idx] for share_idx, share in enumerate(self.shares)]
      # self.protocol = address_qubits(self.protocol)
    for share_idx, share in enumerate(self.shares):
      if self.is_cheater:
  ...
  def retrieve(self, num_bobs: int) -> Program:
    ...
    print("BOB HERE: ", wf_sim.wavefunction(address_qubits(self.protocol)))
